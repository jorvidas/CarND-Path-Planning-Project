...
// Tunable parameters
int N = 20;      // Number of time steps predicting forward
double time_step_size = 0.02; // Time between steps in s 

//FUNCTIONS TO DELETE LATER
void printPredictions(map<int, vector<vector<double> > > predicts) {
    // loop over each ID
    for (auto it = predicts.begin(); it!=predicts.end(); it++) {
        cout << "Car: " << it->first << "\n";
        // loop over each timestep
        vector<vector<double> > car_predictions = it->second;
        for (int i = 0; i < car_predictions.size(); i++) {
            cout << "Timestep: " << i;
            // loop and print each element
            for (int j = 0; j < car_predictions[0].size(); j++) {
                cout  << "  " << car_predictions[i][j];
            }
            cout << "\n";
        }
    }
...
map<int, vector<vector<double> > > makePredictions(vector<vector<double> > sf,
                                                      ...
                                                      vector<double> maps_y) 
{
    ...
    for (auto a : sf) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            prediction.insert(prediction.end(), array, array+(sizeof(array)/sizeof(array[0])));
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // predict
            map<int, vector<vector<double> > > predictions = makePredictions(sensor_fusion, map_waypoints_s, map_waypoints_x, map_waypoints_y);
            printPredictions(predictions);

            // print car 0 fusion data
            // cout << sensor_fusion[0] << "\n";
            }
